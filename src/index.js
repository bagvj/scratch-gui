;(function() {
	...
	function onCommand(command, ...args) {
		switch (command) {
			case "new-project":
				...
				setTimeout(() => (kenrobot.view.project = kenrobot.view.getProject()), 1000);
				break;
			case "open-project":
				onOpenProject();
				break;
			case "save-project":
				onSaveProject(false, args[0]);
				break;
	...
	function onOpenProject(name) {
		kenrobot.postMessage("app:projectOpen", "scratch3", name).then(onProjectLoad, err => {
